...
function DrawWindow(gui, z_index, x, y, w, h, title, centered, callback, close_callback)
	...
	if(close_callback ~= nil)then
		...
		if(CustomButton(gui, "sagsadshds", x + (w + 2), bar_y + 1, z_index - 600, 1, "mods/evaisa.streambattle/files/gfx/ui/minimize.png", 0, 0, 0, 0.5))then
			close_callback()
		end
		GuiLayoutEnd( gui )
	...
	GuiEndAutoBoxNinePiece( gui, 0, w + 12, 8, false, 0, "mods/evaisa.streambattle/files/gfx/ui/9piece_window_bar.png", "mods/evaisa.streambattle/files/gfx/ui/9piece_window_bar.png")
